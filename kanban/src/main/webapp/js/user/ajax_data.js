				function chart_list(){                                       
					$.ajax({ url: "chart_data",
                            type:'post'	,data:{sql:sql_list,db_ip:list_db_ip,db_name:list_db_name,db_port:db_port},	
				...
				function basic_num(){                                                  //基本信息看板赋值
				   $.ajax({ url: "chart_data", 
			       type:'post',data:{sql:sql_basic,db_ip:basic_db_ip,db_name:basic_db_name,db_port:db_port},
				   context: document.body, 
				   dateType:"json",
				   success: function(data){
				     var data_json=JSON.parse(data);
		 ...
		 //参数一：json数组长度（data_json），参数二：需要拼接字符字段
		  ...
		  function draw_chart(sql_sentence,object,db_ip,db_name){         //object中定义的值必须跟数据库返回的字段相同
  		       var temp=new Array(); 
	     	  $.ajax({ url:"chart_data",
                            type:'post'	,data:{sql:sql_sentence,db_ip:db_ip,db_name:db_name,db_port:db_port},					
							context: document.body, 
							dateType:"json",
							async: false,
							success: function(data){
